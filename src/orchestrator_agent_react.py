...
class DREventHandlerAgentReAct:
    """LLM-based DR event handler within HEMS, using ReAct pattern."""

    def __init__(self):
        """Initialize the DR event handler agent."""
        self.system_prompt = self._create_system_prompt()
        self.api_key = CEREBRAS_API_KEY
        self.model = os.environ.get('CEREBRAS_MODEL_OVERRIDE', CEREBRAS_MODEL)
        self.temperature = TEMPERATURE
        self.base_url = "https://api.cerebras.ai/v1"
        print(f"[DR Handler] Using model: {self.model}")

    def _build_household_assets_section(self) -> str:
        """Build the household assets section from config.py AVAILABLE_APPLIANCES."""
        lines = ["You manage the following flexible assets:\n"]
        idx = 1
        for appliance_id, spec in AVAILABLE_APPLIANCES.items():
            if appliance_id == "battery":
                cap = spec.get("capacity_kwh", "?")
                ...
                min_soc = spec.get("min_soc_pct", 20)
                eff = int(spec.get("round_trip_efficiency", 0.9) * 100)
                lines.append(f"{idx}. **battery** - Home battery, {cap} kWh capacity, {pwr} kW charge/discharge, min SoC {min_soc}%, {eff}% efficiency")
            else:
                pwr = spec.get("power_rating_kw", "?")
                dur = spec.get("default_duration_minutes", "?")
                slots = int(dur / 15) if isinstance(dur, (int, float)) else "?"
...
You will work through DR event evaluation step-by-step using a Thought-Action-Observation cycle.
...
Reads current battery state (SoC, capacity, power limits, available energy).
Format: `ACTION: GET_BATTERY_STATE`
Use this before calling the battery agent to know exact SoC.

**Action: CALL_AGENT**
Checks if an asset can contribute to the DR event.
Format: `ACTION: CALL_AGENT | agent_name=<name> | user_request=<request>`
Example: `ACTION: CALL_AGENT | agent_name=battery_agent | user_request=DR event requesting 3kW discharge for 17:00-19:00. Battery at 60% SoC, 4.0 kWh available. Assess feasibility.`

**Action: EVALUATE_FEASIBILITY**
Aggregates asset agent results into a feasibility assessment.
Format: `ACTION: EVALUATE_FEASIBILITY`
Note: Call this after checking relevant asset agents.

**Action: EXPLAIN_TO_PROSUMER**
Generates a conversational explanation of the DR event for the prosumer.
Format: `ACTION: EXPLAIN_TO_PROSUMER`
Note: Call this after feasibility evaluation. The explanation will be shown to the prosumer.

**Action: SUBMIT_RESPONSE**
Submits the prosumer's decision back to the aggregator.
Format: `ACTION: SUBMIT_RESPONSE | accepted=<true/false> | commitment_kw=<kw> | appliances=<list> | reasoning=<text>`
Example: `ACTION: SUBMIT_RESPONSE | accepted=true | commitment_kw=3.0 | appliances=battery | reasoning=Prosumer approved battery discharge`

**Action: FINISH**
Completes DR event handling.
Format: `ACTION: FINISH | summary=<your summary>`

### Required Workflow Order

Note: Wholesale electricity prices are NOT needed here. This is explicit demand response -- the aggregator already committed to a market obligation and provides compensation rates directly. The HEMS only needs to assess physical feasibility.

1. GET_BATTERY_STATE (check battery SoC and capacity)
2. CALL_AGENT (call battery_agent with state and DR details)
3. EVALUATE_FEASIBILITY (aggregate results)
4. EXPLAIN_TO_PROSUMER (conversational explanation with SoC impact)
5. [Wait for prosumer response]
6. SUBMIT_RESPONSE (send decision to aggregator)
7. FINISH

**CRITICAL**: After EXPLAIN_TO_PROSUMER, you MUST wait for the prosumer's response before proceeding. Do NOT auto-accept or auto-reject.
"""
        return base_prompt + react_instructions
    ...
    def _execute_action(self, action, context):
        """Execute a DR event handler action and return observation."""
        from aggregator_tools import submit_dr_response

        action_type = action["type"]
        params = action.get("params", {})

        if action_type == "GET_BATTERY_STATE":
            print("\n   [Action] Reading battery state...")
            battery_state_data = get_battery_state()
            if battery_state_data.get("success"):
                context["battery_state"] = battery_state_data
                return (
                    f"Battery state: {battery_state_data['current_soc_pct']}% SoC "
                    f"({battery_state_data['current_soc_kwh']} kWh / {battery_state_data['capacity_kwh']} kWh). "
                    f"Available above min SoC: {battery_state_data['available_energy_kwh']} kWh. "
                    f"Max discharge: {battery_state_data['max_discharge_kw']} kW. "
                    f"PV forecast: {battery_state_data.get('pv_forecast_kwh', 'N/A')} kWh."
                )
            return f"Error: {battery_state_data.get('error', 'Unknown error')}"
            ...
            # Explicit DR context: prices are not needed for feasibility assessment.
            # Provide minimal stub so call_appliance_agent signature is satisfied.
            if "prices_data" not in context:
                from datetime import datetime
                context["prices_data"] = {
                    ...
                    "timeslots": [f"{h:02d}:{m:02d}" for h in range(24) for m in (0, 15, 30, 45)],
                    "prices": [0.0] * 96
                }

            print(f"\n   [Action] Calling {agent_name} for DR feasibility...")
            result = call_appliance_agent(
                agent_name=agent_name,
            ...
            cost = result.get('cost')
            cost_str = f"{cost:.3f} EUR" if cost is not None else "TBD"
            return f"Agent {agent_name}: Can contribute. Recommended slot {result['recommended_slot']}, duration {result['duration_slots']} slots, cost {cost_str}. Reasoning: {result['reasoning'][:150]}"

        elif action_type == "EVALUATE_FEASIBILITY":
            print("\n   [Action] Evaluating DR feasibility...")
            dr_event = context.get("dr_event", {})
            agent_results = context.get("agent_results", {})
            battery_state_ctx = context.get("battery_state", {})

            if not agent_results:
                return "Error: No agent results available. Call asset agents first."
            ...
            # Add battery SoC context if available
            if battery_state_ctx and battery_state_ctx.get("success"):
                available_kwh = battery_state_ctx.get("available_energy_kwh", 0)
                current_soc = battery_state_ctx.get("current_soc_pct", 0)
                soc_info = f" Battery SoC: {current_soc}%, available energy: {available_kwh} kWh above min."
            ...
            context["feasibility"] = {
                ...
                "feasible": feasible,
                "contributing_appliances": contributing,
                "battery_state": battery_state_ctx
            }
        ...
        elif action_type == "SUBMIT_RESPONSE":
            event_id = context.get("dr_event", {}).get("event_id")
            if not event_id:
                return "Error: No DR event in context"
            ...
            commitment_kw = float(params.get("commitment_kw", 0))
            appliances_str = params.get("appliances", "")
            appliances = [a.strip() for a in appliances_str.split(",") if a.strip()]
            reasoning = params.get("reasoning", "")
            ...
            result = submit_dr_response(
                ...
                accepted=accepted,
                commitment_kw=commitment_kw,
                accepted_appliances=appliances,
                reasoning=reasoning,
                conversation_summary=context.get("final_summary", "")
            )
            ...
            summary = params.get("summary", "DR event handling completed.")
            context["final_summary"] = summary
            return "DR event handling complete."

        else:
            return f"Unknown action type: {action_type}"

    def _save_run_data(self, result, context):
        """Save DR event handler run data."""
        from datetime import datetime
        run_data = {
            "timestamp": datetime.now().isoformat(),
            "agent_type": "dr_event_handler",
            "model": self.model,
            "dr_event": context.get("dr_event", {}),
            "success": result["success"],
            "exit_reason": result.get("exit_reason", "finish"),
        ...
        with open(filepath, "w") as f:
            json.dump(run_data, f, indent=2)
        print(f"\n[Saved] DR handler run data: {filepath}")

    def run_dr_response(self, event_id, prosumer_message=None):
        """
        Run DR event handling ReAct workflow.

        Args:
            event_id: The DR event to evaluate
            prosumer_message: Optional prosumer response message (for continuing conversation)
        ...
        print("\n" + "=" * 80)
        print("DR EVENT HANDLER AGENT (ReAct Pattern)")
        print("=" * 80)
        print(f"\nDR Event: {event_id}")
        print("=" * 80)
        ...
        # Build initial prompt with DR event details
        event_description = (
            f"A DR event has been received from the aggregator:\n"
            f"- Event ID: {dr_event['event_id']}\n"
            f"- Type: {dr_event['event_type']}\n"
            f"- Time window: {dr_event['window_start']} - {dr_event['window_end']}\n"
            f"- Target reduction: {dr_event['target_kw']} kW\n"
            f"- Compensation: {dr_event['compensation_eur_kwh']} EUR/kWh\n"
            f"- Maximum earnings: {dr_event.get('max_compensation_eur', 'N/A')} EUR\n\n"
            f"Please evaluate this DR event, check feasibility with household appliances, "
            f"and prepare an explanation for the prosumer."
        )

        if prosumer_message:
            event_description += f"\n\nThe prosumer has responded: {prosumer_message}"
        ...
        print("\n[DR Handler] Starting ReAct workflow...\n")
        ...
        try:
            for iteration in range(max_iterations):
                ...
                # If we just explained to prosumer, pause for prosumer input
                if action["type"] == "EXPLAIN_TO_PROSUMER" and not prosumer_message:
                    # Save conversation history for follow-up turns
                    messages.append({"role": "user", "content": f"Observation: {observation}"})
                    self._save_conversation_history(event_id, messages)
    ...
    def run_dr_followup(self, event_id, prosumer_message):
        """
        Handle a follow-up from the prosumer during DR negotiation.
        Uses a mini ReAct loop so the agent can execute tools (CALL_AGENT,
        GET_BATTERY_STATE, etc.) before responding, not just SUBMIT_DR_RESPONSE.
        ...
        followup_prompt = (
            f"The prosumer has responded about the DR event: {prepared_message}\n\n"
            f"You have all the context from the evaluation above. "
            f"If the prosumer asks you to run a tool or check something, DO IT using the "
            f"appropriate ACTION. You have access to these actions:\n"
            f"  ACTION: GET_BATTERY_STATE\n"
            f"  ACTION: CALL_AGENT | agent_name=battery_agent | user_request=<request>\n"
            f"  ACTION: EVALUATE_FEASIBILITY\n"
            f"  ACTION: EXPLAIN_TO_PROSUMER\n"
            f"  ACTION: SUBMIT_DR_RESPONSE | accepted=true | commitment_kw=<kw> | appliances=<list> | reasoning=<text>\n"
            f"  ACTION: SUBMIT_DR_RESPONSE | accepted=false | commitment_kw=0 | appliances= | reasoning=<text>\n"
            f"  ACTION: FINISH | summary=<summary>\n\n"
            f"Rules:\n"
            f"- Output exactly ONE action per turn, then STOP. Wait for the Observation before continuing.\n"
            f"- NEVER write a fake Observation yourself. The system provides observations.\n"
            f"- If the prosumer asks a question you can answer from context, respond without any ACTION.\n"
            f"- If the prosumer approves/rejects, use SUBMIT_DR_RESPONSE then FINISH on the next turn.\n"
            f"- If the prosumer asks to run the battery agent or check feasibility, use CALL_AGENT.\n"
            f"- After completing tool actions, explain results to the prosumer (EXPLAIN_TO_PROSUMER) then STOP."
        )
        messages.append({"role": "user", "content": followup_prompt})
        ...
        # Mini ReAct loop (max 5 iterations)
        max_iterations = 5
        for iteration in range(max_iterations):
            print(f"\n[Followup] Iteration {iteration + 1}/{max_iterations}")
            ...
            # No action -- pure conversational reply, stop the loop
            if not action:
                final_reply = llm_response
                messages.append({"role": "assistant", "content": llm_response})
                break

            # FINISH or EXPLAIN_TO_PROSUMER -- terminal actions
            if action["type"] in ("FINISH", "EXPLAIN_TO_PROSUMER"):
                observation = self._execute_action(action, context)
                print(f"[Followup] {observation}")
                actions_taken.append({"iteration": iteration + 1, "action": action, "observation": observation})
                final_reply = llm_response
                messages.append({"role": "assistant", "content": llm_response})
                break

            # SUBMIT_DR_RESPONSE -- execute, then look for FINISH on next turn
            if action["type"] == "SUBMIT_DR_RESPONSE":
                observation = self._execute_action(action, context)
                print(f"[Followup] {observation}")
                actions_taken.append({"iteration": iteration + 1, "action": action, "observation": observation})
                final_reply = llm_response
                messages.append({"role": "assistant", "content": llm_response})
                # Continue loop so the LLM can output FINISH
                messages.append({"role": "user", "content": f"Observation: {observation}\n\nNow output ACTION: FINISH with a summary."})
                continue

            # All other actions (CALL_AGENT, GET_BATTERY_STATE, EVALUATE_FEASIBILITY, etc.)
            observation = self._execute_action(action, context)
            print(f"[Followup] {observation}")
            actions_taken.append({"iteration": iteration + 1, "action": action, "observation": observation})

            # Append assistant response + observation, continue loop
            messages.append({"role": "assistant", "content": llm_response})
            messages.append({"role": "user", "content": f"Observation: {observation}"})

        else:
            # Exhausted iterations -- use last response
            final_reply = llm_response if llm_response else "Reached maximum follow-up iterations."

        # Reload event from disk to capture any status changes
        with open(event_path, 'r') as f:
            dr_event = json.load(f)

        # Save updated conversation history
        dr_event["conversation_history"] = messages
        turn_count = sum(1 for m in messages if m["role"] == "user") - 1
